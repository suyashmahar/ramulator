...
namespace ramulator
{
    ...
    public:
        ...
        inline bool is_cmdlegal(Command cmd, Level level, State state) {
            ...
            // Any of the RWA commands cannot be issued while a rank is powered down
            if (is_cmd_rwa) {
                if (level == Level::Rank) {
                    switch (int(state)) {
                        ...
                        case (int(State::SActPowerDown)):
                            return false; // Command is not legal
                        default:
                            return true; // It is legal and can be issued
                    }
                }
            }
            return true;
